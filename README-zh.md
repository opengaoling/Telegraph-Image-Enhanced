...
2、后台管理页面支持自定义 UI 登录验证功能（已移除原生的 Basic Auth 弹窗，采用内置登录页），默认关闭，如需开启请部署完成后前往后台依次点击`设置`->`环境变量`->`为生产环境定义变量`->`编辑变量` 添加如下表格所示的变量即可开启登录验证
| 变量名称 | 值 |
| ----------- | ----------- |
|ADMIN_USER = | <后台管理页面登录用户名称>|
|ADMIN_PASS = | <后台管理页面登录用户密码>|
